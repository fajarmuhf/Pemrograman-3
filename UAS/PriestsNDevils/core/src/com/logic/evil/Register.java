...
public class Register implements Screen{
	...
	public void show() {
		...
		gambar = game.assets.get("img/register/register.pack"); 
        skinGambar = new Skin();
        ...
        okButton.addListener(new InputListener() {
        	...
        	public void touchUp (InputEvent event, float x, float y, int pointer, int button) {
        		if(x >= (event.getTarget()).getX() && x < (event.getTarget()).getX()+(event.getTarget()).getWidth() && y >= (event.getTarget()).getY() && y <= (event.getTarget()).getY()+(event.getTarget()).getHeight()){
        	        ...
        	        boolean hasil = db.cekUsername(game.pengguna);
        	        if(hasil){
        	        	game.pengguna.setTanggal(Calendar.getInstance().getTime());
        	        	db.tambahPlayer(game.pengguna);
        	        	HistoryRegister hr = new HistoryRegister();
        	        	hr.setIdPlayer(db.getIdByUsername(game.pengguna));
        	        	hr.setTanggal(Calendar.getInstance().getTime());
        	        	db.tambahHistoryRegister(hr);
        	        	keteranganText.setText("register sucess");
        	        	...
        	        	keteranganText.setText("register failure");
        	        	float fontX = (360*Gdx.graphics.getWidth()/960) + (passwordText.getWidth()/2) - ((font.getBounds(passwordText.getText()).width)/2);
        	    	    float fontY = 80*Gdx.graphics.getHeight()/540;
        	    	    keteranganText.setPosition(fontX, fontY);
            	        db.closeDB();
        ...
        usernameButton.addListener(new InputListener() {
        	public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
        		...
        		game.setScreen(new InputSecond(game));
                return true;
        	};
        	public void touchUp (InputEvent event, float x, float y, int pointer, int button) {
        		if(x >= (event.getTarget()).getX() && x < (event.getTarget()).getX()+(event.getTarget()).getWidth() && y >= (event.getTarget()).getY() && y <= (event.getTarget()).getY()+(event.getTarget()).getHeight()){
