...
public class Score implements Screen{
	...
	TextFieldStyle skorTextStyle;
	TextArea skorText;
	Sound clickSound;
	float soundVolume;
	EvilMain game;
	
	Score(EvilMain Permainan){
		game = Permainan;
	}
	...
	public void show() {
		...
		gambar = game.assets.get("img/score/score.pack"); 
        skinGambar = new Skin();
        ...
        skorTextStyle = new TextFieldStyle();
        skorTextStyle.font = font;
        skorTextStyle.fontColor = Color.WHITE;
        ...
        skorText = new TextArea("", skorTextStyle);
        skorText.setPosition(190*Gdx.graphics.getWidth()/960, 120*Gdx.graphics.getHeight()/540); 
        skorText.setHeight(274*Gdx.graphics.getHeight()/540); 
        skorText.setWidth(601*Gdx.graphics.getWidth()/960); 
        skorText.setCenterPosition(690*Gdx.graphics.getWidth()/960, 210*Gdx.graphics.getHeight()/540);
        
        DatabaseEvil db = new DatabaseEvil();
        List<SkorDetail> list = db.tampilSkor();
        String textSkor = "List Score\n";
        for(int i=0;i<list.size();i++){
        	textSkor += (i+1)+". "+list.get(i).getUsername()+" : ";
        	textSkor += list.get(i).getSkor()+"\n";
        }
        skorText.setText(textSkor);
        db.closeDB();
